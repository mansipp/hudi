...
public class SparkHoodieDynamoDBIndex extends HoodieIndex<Object, Object> {
  ...
  protected String dynamoDBPartitionKey = "recordKey";
  protected String dynamoDBSortKey = "partitionPath";
  ...
  public void close() {
    ...
    LOG.info("Released the DynamoDB resources.");
  }

  protected void checkIfTableExists(AmazonDynamoDB dynamoDB, HoodieWriteConfig hoodieWriteConfig) {
    if (!indexTableExists(dynamoDB)) {
      ...
      LOG.info("Table " + tableName + " exists in DynamoDB.");
    }
  }
  ...
  protected void createTableInDynamoDB(AmazonDynamoDB dynamoDB, HoodieWriteConfig hoodieWriteConfig) {
    LOG.info("Creating Amazon DynamoDB index table named " + tableName);
    if (dynamoDB == null) {
    ...
    if (billingMode.equals(BillingMode.PROVISIONED.name())) {
      request.withProvisionedThroughput(new ProvisionedThroughput()
          ...
          .withWriteCapacityUnits(Long.parseLong(hoodieWriteConfig.getDynamoDBIndexWriteCapacity())));
    }
